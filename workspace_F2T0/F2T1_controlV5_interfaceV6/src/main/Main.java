...
public class Main {
	...
	public static boolean soundSources=true;					// activate sound sources
	
	public static int LENGTH=10000;								// length of the trace
	
	static {
		if (System.getProperty("os.name").toLowerCase().indexOf("win") >= 0){
			try {
				String directory = new File(".").getCanonicalPath();
				System.load(directory+"\\soft_oal.dll");
				System.out.println("soft_oal.dll loaded");
		    } catch (Exception e) {
	...
	public Main(){
		...
		if (!soundSources) println("sound sources: dehactivated");
	...
	public void listFiles(){
		...
		else println("Source file directory does not exist");
		...
		else println("Symbol file directory does not exist");
		symbolList.clear();
	...
	public void saveSource(String name){
		if (currentAge.sourceList.size()==0) println("No sound source to save");
		else{
			String fileName = FILES+SOURCE+name;
			try {
				PrintWriter file  = new PrintWriter(new FileWriter(fileName));
				for (int i=0;i<currentAge.sourceList.size();i++){
					file.println("s "+currentAge.sourceList.get(i).px+" "+currentAge.sourceList.get(i).py+" "+currentAge.sourceList.get(i).filename+" "+currentAge.sourceList.get(i).filename2);
				}
				file.close();
				println("source configuration saved : "+name);
			}
			catch (Exception e) {e.printStackTrace();}
		}
	}
	...
	private void loadSymbol(String file){
			...
			while (line!=null){
				...
				if (elements.length>=5 && elements[0].equals("t")){
					if (elements.length<7)
						symbolList.get(symb).setTarget(new TargetPoint(Integer.parseInt(elements[1]), Integer.parseInt(elements[2]),
		...
		catch (Exception e) {println("   /!\\ Path file not found or containing errors");}
	}
	...
	public static void main(String[] args){
		...
		try {
			...
			while (line!=null){
				...
				else if (elements.length>=2 && elements[0].equals("soundsources")){
					if (elements[1].equals("0")) soundSources=false;
					else soundSources=true;
				}
				line=br.readLine();
			}
			br.close();
