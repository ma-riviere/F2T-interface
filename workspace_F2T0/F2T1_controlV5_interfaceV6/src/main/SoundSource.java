 ...
 * @authors Michał Bujacz, simon gay
 */
...
//                System.out.println("scaled   " +scaled_rx + " " + scaled_ry + " " + scaled_dist);
	...
	public void setParams(String[] args){
        ...
        if (args.length > 3){
        	if (args[3].lastIndexOf('.')==-1) filename = args[3]+".wav"; 
        	else filename = ""+args[3];
        }
        else                 filename = "synth_15.wav"; 
     
        if (args.length > 4){
        	if (args[4].lastIndexOf('.')==-1) filename2 = args[4]+".wav";
        	else filename2 = ""+args[4];
        }
