...
public class DisplayAreaEditPanel  extends JPanel implements ActionListener, MouseListener, MouseMotionListener, KeyListener{
	...
	private BufferedImage img = new BufferedImage( 705, 705, BufferedImage.TYPE_INT_RGB );
	...
	public void paintComponent(Graphics g){
		...
		g.fillRect(700, 0, this.getWidth()-700, this.getHeight());
		...
		if (selected_image==0){
			if (main.script.ageList.get(main.script.currentAge).image.view!=null){
				if (!main.script.ageList.get(main.script.currentAge).image.view.equals(displayed) || change){
					back=main.script.ageList.get(main.script.currentAge).image.view_img;
					setImage(main.script.ageList.get(main.script.currentAge).image.view_img);
					change=true;
			...
			if (main.script.ageList.get(main.script.currentAge).image.tactile!=null){
				if (!main.script.ageList.get(main.script.currentAge).image.tactile.equals(displayed) || change){
					back=main.script.ageList.get(main.script.currentAge).image.tactile_img;
					setImage(main.script.ageList.get(main.script.currentAge).image.tactile_img);
					change=true;
			...
			if (main.script.ageList.get(main.script.currentAge).image.flow!=null){
				if (!main.script.ageList.get(main.script.currentAge).image.flow.equals(displayed) || change){
					back=main.script.ageList.get(main.script.currentAge).image.flow_img;
					setImage(main.script.ageList.get(main.script.currentAge).image.flow_img);
					change=true;
			...
			if (main.script.ageList.get(main.script.currentAge).image.rail!=null){
				if (!main.script.ageList.get(main.script.currentAge).image.rail.equals(displayed) || change){
					back=main.script.ageList.get(main.script.currentAge).image.rail_img;
					setImage(main.script.ageList.get(main.script.currentAge).image.rail_img);
					change=true;
			...
			if (main.script.ageList.get(main.script.currentAge).image.area!=null){
				if (!main.script.ageList.get(main.script.currentAge).image.area.equals(displayed) || change){
					back=main.script.ageList.get(main.script.currentAge).image.area_img;
					setImage(main.script.ageList.get(main.script.currentAge).image.area_img);
					change=true;
			...
			if (main.script.ageList.get(main.script.currentAge).image.magnetic!=null){
				if (!main.script.ageList.get(main.script.currentAge).image.magnetic.equals(displayed) || change){
					back=main.script.ageList.get(main.script.currentAge).image.magnetic_img;
					setImage(main.script.ageList.get(main.script.currentAge).image.magnetic_img);
					change=true;
	...
	public void actionPerformed(ActionEvent e) {
			...
			if (main.script.ageList.get(main.script.currentAge).image.area!=null){
				...
				for (int i=0;i<700;i++){
					for (int j=0;j<700;j++){
						...
						if (displayed.equals("") || i%2==0 && j%2==0){
							if (red_channel) r=area_map[i][j][0];
							else r=0;
							if (green_channel) g=area_map[i][j][1];
							else g=0;
							if (blue_channel) b=area_map[i][j][2];
							else b=0;
		...
		else if (e.getSource()==red_box){
			red_channel=red_box.isSelected();
			setImage(back);
		}
		else if (e.getSource()==green_box){
			green_channel=green_box.isSelected();
			setImage(back);
		}
		else if (e.getSource()==blue_box){
			blue_channel=blue_box.isSelected();
			setImage(back);
		}
		
		else if (e.getSource()==areaLoad) loadArea((String)list_area.getSelectedItem());
	...
	public void loadArea(String file){
		main.println("Load area file "+file+" in area editor :");
		...
		catch (Exception e) {main.println("   /!\\ Area file not found or containing errors");}
	...
	public void saveArea(String name){
		...
		try {
		    ImageIO.write( bufferedImage, "png", new File(Main.FILES+Main.AREA+name+".png") ); }
		     ...
		     main.println("errorr while saving image..." );
		     e.printStackTrace();
		}
	...
	public void setImage(BufferedImage back) {
		...
		if (back!=null) {
			for (int i=0;i<700;i++){
				for (int j=0;j<700;j++){
					img.setRGB(i, j, back.getRGB(i, j));
				}
			}
