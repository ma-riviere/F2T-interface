...
public class PicturePanel extends JPanel implements MouseListener, MouseMotionListener{
	...
	public void paintComponent(Graphics g){
		...
		if (main.path.size()>0){
			...
			int vx1=(main.path.get(0).x);
			int vy1=(main.path.get(0).y);
			
			g.fillOval(vx1+550-4, 350-vy1-4, 10, 10);
			
			if (selected_point==0){
				g.setColor(Color.magenta);
				g.drawOval(vx1+550-7, 350-vy1-7, 16, 16);
			}
			
			for (int i=1;i<main.path.size();i++){
				
				vx1=main.path.get(i-1).x;
				vy1=main.path.get(i-1).y;
				int vx2=main.path.get(i  ).x;
				int vy2=main.path.get(i  ).y;
				
				if (main.path.get(i).control==0) g.setColor(Color.yellow);
				else g.setColor(Color.magenta);
				g.drawLine(vx1+550, 350-vy1, vx2+550, 350-vy2);
				g.fillOval(vx2+550-4, 350-vy2-4, 10, 10);
				
				if (selected_point==i){
					g.setColor(Color.magenta);
					g.drawOval(vx2+550-7, 350-vy2-7, 16, 16);
				}
			}	
		}
		///////////////////////////////////////////////////////////////////////////////////
