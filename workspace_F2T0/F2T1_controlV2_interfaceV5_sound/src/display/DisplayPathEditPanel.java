...
public class DisplayPathEditPanel  extends JPanel implements ActionListener, MouseListener, MouseMotionListener, KeyListener{
	...
	public DisplayPathEditPanel(Main m, DisplayFrame f){
		...
		/////////////////////////////////
		path=new ArrayList[NBSLOTS];
		path_buffer=new ArrayList[NBSLOTS];
		buffer=new int[NBSLOTS];
		
		for (int i=0;i<NBSLOTS;i++){
			path[i]=new ArrayList<Target>();
			path_buffer[i]=new ArrayList<ArrayList<Target>>();
			path_buffer[i].add(new ArrayList<Target>());
			buffer[i]=0;
		}
	}
	...
	public void paintComponent(Graphics g){
		...
		if (path[selected_slot].size()>0){
			
			int speed=path[selected_slot].get(0).speed/2;
			if (speed>255) speed=255;
			if (speed<0) speed=0;
			
			if (path[selected_slot].get(0).control==0) g.setColor(new Color(speed,255-speed,255));
			else g.setColor(new Color(speed,255-speed/2,0));
			
			int vx1=(path[selected_slot].get(0).x);
			int vy1=(path[selected_slot].get(0).y);
			...
			for (int i=1;i<path[selected_slot].size();i++){
				
				vx1=path[selected_slot].get(i-1).x;
				vy1=path[selected_slot].get(i-1).y;
				int vx2=path[selected_slot].get(i  ).x;
				int vy2=path[selected_slot].get(i  ).y;
				
				speed=path[selected_slot].get(i).speed/2;
				if (speed>255) speed=255;
				if (speed<0) speed=0;
				if (path[selected_slot].get(i).control==0) g.setColor(new Color(speed,255-speed,255));
				else g.setColor(new Color(speed,255-speed/2,0));
				g.drawLine(vx1+350, 350-vy1, vx2+350, 350-vy2);
				g.fillOval(vx2+350-4, 350-vy2-4, 10, 10);
				
				if (selected_point==i){
					g.setColor(Color.magenta);
					g.drawOval(vx2+350-7, 350-vy2-7, 16, 16);
				}
			}	
		}
		...
		if (selected_point>=0 && selected_point<path[selected_slot].size()){
			g.drawString("position : ("+path[selected_slot].get(selected_point).x+", "+path[selected_slot].get(selected_point).y+")", 830, 160);
			
			if (path[selected_slot].get(selected_point).control==1) 
				g.drawString("speed : "+path[selected_slot].get(selected_point).speed+", type : control", 830, 180);
			else
				g.drawString("force : "+path[selected_slot].get(selected_point).speed+", type : attractor", 830, 180);
		}
		
		for (int i=0;i<NBSLOTS;i++){
			if (buffer[i]==0 && path[i].size()==0) g.drawString("empty", 980, 20+20*i);
			else g.drawString(path[i].size()+" points", 980, 20+20*i);
		}	
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource()==remove){
			if (selected_point>=0 && selected_point<path[selected_slot].size()){
				...
				if (selected_point>=path[selected_slot].size()) selected_point--;
				pushBuffer();
			}
		}
		else if (e.getSource()==add){
			if (selected_point>0 && selected_point<path[selected_slot].size()){
				int x1=(path[selected_slot].get(selected_point).x + path[selected_slot].get(selected_point-1).x )/2;
				int y1=(path[selected_slot].get(selected_point).y + path[selected_slot].get(selected_point-1).y )/2;
			...
			if (selected_point>=0 && selected_point<path[selected_slot].size()){
				path[selected_slot].get(selected_point).speed=main.target_speed;
				path[selected_slot].get(selected_point).control=main.target_type;
			}
			pushBuffer();
		}
			...
			if (buffer[selected_slot]>0){
				path[selected_slot].clear();
				for (int i=0;i<path_buffer[selected_slot].get(buffer[selected_slot]-1).size();i++){
					path[selected_slot].add(path_buffer[selected_slot].get(buffer[selected_slot]-1).get(i).duplicate());
				}
			...
			for (int i=0;i<path[selected_slot].size();i++){
				main.script.ageList.get(main.script.currentAge).targetSequence.add(path[selected_slot].get(i).duplicate());
			}
		}
		else if (e.getSource()==get){
			for (int i=0;i<main.script.ageList.get(main.script.currentAge).targetSequence.size();i++){
				path[selected_slot].add(main.script.ageList.get(main.script.currentAge).targetSequence.get(i).duplicate());
			}
		}
		
		else if (e.getSource()==pathLoad) loadPath((String)list_path.getSelectedItem());
		else if (e.getSource()==pathSave){
			if (name.getText().length()>0) savePath(name.getText());
		}
	...
	public void mousePressed(MouseEvent e) {
		...
		if (click_right){
			...
			if (tx>-345 && tx<345 && ty>-345 && ty<345){
				...
				for (int i=0;i<path[selected_slot].size();i++){
					d=Math.sqrt( (tx-(path[selected_slot].get(i).x))*(tx-(path[selected_slot].get(i).x)) + (ty-(path[selected_slot].get(i).y))*(ty-(path[selected_slot].get(i).y)));
					if (d<min){
						min=d;
						imin=i;
					}
	...
	public void mouseDragged(MouseEvent e) {
		...
		if (click_right && point_capture){
			if (path[selected_slot].get(selected_point).x!=tx || path[selected_slot].get(selected_point).y!=ty) write=true;
			path[selected_slot].get(selected_point).x=tx;
			path[selected_slot].get(selected_point).y=ty;
		}
		this.repaint();
	}
	...
	public void pushBuffer(){	
		...
		path_buffer[selected_slot].add(new ArrayList<Target>());
		buffer[selected_slot]++;
		for (int i=0;i<path[selected_slot].size();i++){
			path_buffer[selected_slot].get(buffer[selected_slot]).add(path[selected_slot].get(i).duplicate());
		}
	...
	public void savePath(String name){
		...
		try {
			...
			for (int t=0;t<path[selected_slot].size();t++){
				file.println("t "+path[selected_slot].get(t).x+" "+path[selected_slot].get(t).y+" "+path[selected_slot].get(t).speed+" "+path[selected_slot].get(t).control);
			}
	...
	public void keyReleased(KeyEvent e) {
		...
		if (e.getSource()==name){
			...
			while (!found && i<main.listPath.length){
				if (main.listPath[i].equals(name.getText())) found=true;
				i++;
			}
			...
			if (name.getText().length()==0) pathSave.setEnabled(false);
