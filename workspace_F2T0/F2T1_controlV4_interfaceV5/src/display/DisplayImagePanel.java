	...
	public void paintComponent(Graphics g){
			...
			if (frame.selected_image==0){
				...
				if (main.script.ageList.get(main.script.currentAge).image.magnetic!=null){
					...
					//g.fillOval(350+(int)main.virtual.magnetic.xmin-3, 350-(int)main.virtual.magnetic.ymin-3, 7, 7);
				}
				else{
					g.setColor(Color.gray);
					g.fillRect(0, 0, 699, 699);
			...
			if (frame.selected_image>=0){
				...
				if (frame.displayTrace){
		 		...
		 		g.drawLine(425+(int)(main.x/3), 420-(int)(main.y/2.56),425+(int)(main.x/3)+(int)main.jx*5, 420-(int)(main.y/2.56)-(int)main.jy*5);
			}
			...
			if (main.script.ageList.get(main.script.currentAge)!=null && main.script.ageList.get(main.script.currentAge).targetSequence.sizeTarget()>0){
				...
				for (int i=1;i<main.script.ageList.get(main.script.currentAge).targetSequence.sizeTarget();i++){
					...
					//g.drawLine(350+(int)main.script.ageList.get(main.script.currentAge).targetSequence.targetList.get(i-1).x, 350-(int)main.script.ageList.get(main.script.currentAge).targetSequence.targetList.get(i-1).y,
					//		   350+(int)main.script.ageList.get(main.script.currentAge).targetSequence.targetList.get(i  ).x, 350-(int)main.script.ageList.get(main.script.currentAge).targetSequence.targetList.get(i  ).y);
