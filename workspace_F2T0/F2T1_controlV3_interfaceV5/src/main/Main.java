...
public class Main {
	...
	public void controlLoop(){
		...
		DdeltaX=deltaX - deltaX_prev2;
		DdeltaY=deltaY - deltaY_prev2;
		...
		else*/ vmX-=10*deltaX + 20*DdeltaX;
		
		/*if (deltaY>30) vmY+=254;
		else if (deltaY<-30) vmY-=254;
		else*/ vmY-=10*deltaY + 20*DdeltaY;
		
		
		//for (int i=0;i<virtual.moduleList.size();i++) virtual.moduleList.get(i).compute2();
		
		if (vmX*vmX+vmY*vmY<2500){
			vmX=0;
			vmY=0;
		}
		
		if (vmX<20 && vmX>-20) vmX=0;
		if (vmY<20 && vmY>-20) vmY=0;
