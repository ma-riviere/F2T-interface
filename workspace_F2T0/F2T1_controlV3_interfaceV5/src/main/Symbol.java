...
public class Symbol {
	...
	public ArrayList<Target> sequence;
	public ArrayList<Target> areas;
	...
	public Symbol(Main m, String n){
		sequence=new ArrayList<Target>();
		areas=new ArrayList<Target>();
	...
	public void set(int px, int py){
		...
		if (px+maxX<344 && px+minX>-345 && py+maxY<349 && py+minY>-345){
			...
			for (int i=0;i<sequence.size();i++){
				main.currentAge.targetSequence.add(index, new Target(sequence.get(i).x+px, sequence.get(i).y+py, sequence.get(i).speed, sequence.get(i).control));
				index++;
			}
		}
	...
	public void setTarget(Target t){
		...
		if (t.control==1){
			...
			for (int i=0;i<sequence.size();i++){
				if (sequence.get(i).x+20>maxX) maxX=sequence.get(i).x+20;
				if (sequence.get(i).x-20<minX) minX=sequence.get(i).x-20;
				if (sequence.get(i).y+20>maxY) maxY=sequence.get(i).y+20;
				if (sequence.get(i).y-20<minY) minY=sequence.get(i).y-20;
			}
		}
		else{
			areas.add(t);
