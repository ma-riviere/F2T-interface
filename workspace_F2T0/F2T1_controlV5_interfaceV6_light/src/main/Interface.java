...
/**
 * Interface between the software and hardware tablet. The interface handle messages from the tablet and send motor commands
 * @author simon gay
 */
	...
	public Interface(Main m){
		...
		try {
			...
			F2T_connected=true;
				...
				main.println("cannot connect port USB"+port);
				port++;
			}
		}
                ...
                for(int i=0;i<received.length; i++){
                	...
                	if ((received[i] & 0xff)==255){
                		...
                		if (message_type==1){
                			...
                			if (joystickX*joystickX+joystickY*joystickY>21.16){ // 4.6²
                				float norm=(float) Math.sqrt(joystickX*joystickX+joystickY*joystickY);
                				joystickX=joystickX*4.6f/norm;
                				joystickY=joystickY*4.6f/norm;
                			}
