...
public class Areas {
	...
	public void detect(int i, int j){
		...
		if (age.image.area_mat[i][j][0]!=0 || age.image.area_mat[i][j][1]!=0 || age.image.area_mat[i][j][2]!=0){
			
			int red=age.image.area_mat[i][j][0];
			int green=age.image.area_mat[i][j][1];
			int blue=age.image.area_mat[i][j][2];
