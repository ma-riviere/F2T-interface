...
public class DisplayFlowEditPanel  extends JPanel implements ActionListener, MouseListener, MouseMotionListener, KeyListener{
	...
	public void paintComponent(Graphics g){
		...
		for (int i=0;i<700;i+=2){
			for (int j=0;j<700;j+=2){
				r= Math.max(0, Math.min(255, main.currentAge.image.flow_mat[i][j][0]));
				green= Math.max(0, Math.min(255, main.currentAge.image.flow_mat[i][j][1]));
				
				g.setColor(new Color(r,green,0));
				g.fillRect(i, j, 2, 2);
			}
		}*/
	...
	public void saveFlow(String name){
		...
		System.out.println("save flow : "+name);
	...
	public void saveRail(String name){
		...
		System.out.println("save rail : "+name);
