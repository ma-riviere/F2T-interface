...
import structures.BezierCurvePath;
...
public class DisplayPathEditPanel  extends JPanel implements ActionListener, MouseListener, MouseMotionListener, KeyListener{
	...
	public ArrayList<BezierCurvePath>[] path_buffer;
	public int buffer[];
	...
	public DisplayPathEditPanel(Main m, DisplayFrame f){
		...
		/////////////////////////////////
		path=new BezierCurvePath[NBSLOTS];
		path_buffer=new ArrayList[NBSLOTS];
		buffer=new int[NBSLOTS];
		
		for (int i=0;i<NBSLOTS;i++){
			path[i]=new BezierCurvePath();
			path_buffer[i]=new ArrayList<BezierCurvePath>();
			path_buffer[i].add(new BezierCurvePath());
			buffer[i]=0;
		}
	}
	...
	public void paintComponent(Graphics g){
		...
		if (selected_point>=0 && selected_point<path[selected_slot].sizeTarget()){
			...
			if (path[selected_slot].targetList.get(selected_point).control==1) 
				g.drawString("speed: "+path[selected_slot].targetList.get(selected_point).speed+
						", type: control , Bezier point: ("+path[selected_slot].targetList.get(selected_point).p1x+", "+path[selected_slot].targetList.get(selected_point).p1y+")", 730, 180);
			else
				g.drawString("force: "+path[selected_slot].targetList.get(selected_point).speed+
						", type: attractor , Bezier point: ("+path[selected_slot].targetList.get(selected_point).p1x+", "+path[selected_slot].targetList.get(selected_point).p1y+")", 730, 180);
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource()==remove){
			...
			if (selected_point>0 && selected_point<path[selected_slot].sizeTarget()){
				...
				path[selected_slot].add(selected_point, new TargetPoint(x1, y1,main.target_speed, main.target_type));
				pushBuffer();
			}
		}
		...
		if (click_left){
			if (!e.isShiftDown()){
				...
				if (tx>-345 && tx<345 && ty>-345 && ty<345){
					path[selected_slot].add(new TargetPoint(tx, ty,main.target_speed, main.target_type));
					pushBuffer();
				}
			}
			else{
	...
	public void mouseDragged(MouseEvent e) {
		...
		if (click_right && selected_point>=0 && selected_point<path[selected_slot].sizeTarget() && e.isShiftDown()){
			path[selected_slot].setBezier(selected_point,ex-350, -ey+350);
			write=true;
		}
	...
	public void pushBuffer(){	
		...
		path_buffer[selected_slot].add(new BezierCurvePath());
		buffer[selected_slot]++;
		for (int i=0;i<path[selected_slot].sizeTarget();i++){
			...
			while (line!=null){
				...
				if (elements.length>=5 && elements[0].equals("t")){
					if (elements.length<7)
						path[selected_slot].add(new TargetPoint(Integer.parseInt(elements[1]), Integer.parseInt(elements[2]),
	...
	public void loadSymbol(String file){
		...
		try {
			...
			while (line!=null){
				...
				if (elements.length>=5 && elements[0].equals("t")){
					if (elements.length<7)
						path[selected_slot].add(i,new TargetPoint(Integer.parseInt(elements[1]), Integer.parseInt(elements[2]),
													     	 Integer.parseInt(elements[3]), Integer.parseInt(elements[4]) ));
					else
						path[selected_slot].add(new TargetPoint(Integer.parseInt(elements[1]), Integer.parseInt(elements[2]),
														   Integer.parseInt(elements[3]), Integer.parseInt(elements[4]), 
