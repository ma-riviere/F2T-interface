...
public class Script {
	...
	public void loadScript(String file){
		...
		try {
			...
			while (line!=null){
				...
				else if (elements.length>=5 && elements[0].equals("t")) ageList.get(writing_age).history.get(writing_history).path.add(new Target(elements));
		...
		try {
			...
			while (line!=null){
				...
				if (elements.length>0){
					...
					if (elements.length>=5 && elements[0].equals("t")) writingPeriod.path.add(new Target(elements));
		...
		try {
			...
			while (line!=null){
				...
				if (elements.length==5 && elements[0].equals("t")){
					writingPeriod.path.add(new Target(Integer.parseInt(elements[1]), Integer.parseInt(elements[2]),
	...
	public void setPreset(String file, int erase){
		...
		if (erase>=2){
			...
			ageList.get(currentAge).areas.close();
		}
		...
		try {
			...
			while (line!=null){
				...
				if (elements.length>0){
					...
					else if (elements.length>=5 && elements[0].equals("t")) ageList.get(currentAge).targetSequence.add(new Target(elements));
	...
	public void setPath(String file){
		...
		try {
			...
			while (line!=null){
				...
				elements=line.split(" ");
				if (elements.length==5 && elements[0].equals("t")){
					ageList.get(currentAge).targetSequence.add(new Target(Integer.parseInt(elements[1]), Integer.parseInt(elements[2]),Integer.parseInt(elements[3]), Integer.parseInt(elements[4]) ));
				}
				line=br.readLine();
			}
			br.close();
	...
	public void setCommandLine(String line){
		...
		try {
			...
			if (     elements.length>=2 && elements[0].equals("image")){
				...
				System.out.println("test "+elements[1]);
			}
			else if (elements.length>=2 && elements[0].equals("tactile")){
				if (elements[0].equals("none")) ageList.get(currentAge).setTactile(null);
				else							ageList.get(currentAge).setTactile(elements[1]);
